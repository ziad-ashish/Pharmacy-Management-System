...
<div class="page active" id="page-settings">
  ...
  function _renderPreview(tab, draft = {}) {
    ...
    const previews = {
      ...
      invoice: `<div class="sp-label">معاينة الفاتورة</div><div class="sp-receipt"><div class="sp-r-head"><strong>${name}</strong><small>فاتورة بيع تجريبية</small></div><div class="sp-r-row"><span>دواء تجريبي</span><b>100.00</b></div>${v.showTax!=='0'?`<div class="sp-r-row"><span>الضريبة ${_esc(v.taxRate)||0}%</span><b>${Number(v.taxRate||0).toFixed(2)}</b></div>`:''}<div class="sp-r-total"><span>الإجمالي</span><b>${(100+Number(v.taxRate||0)).toFixed(2)} ${_esc(v.currency)||'ر.س'}</b></div><p>${_esc(v.invoiceNote)||'شكرًا لتعاملكم معنا'}</p></div>`,
